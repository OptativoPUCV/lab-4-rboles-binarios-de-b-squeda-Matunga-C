...
/*La función Pair* upperBound(TreeMap* tree, void* key) retorna el **Pair** con clave igual a key. En caso de no encontrarlo retorna el 
primer par asociado a una clave mayor o igual a key. 
Para implementarla puede realizar una búsqueda normal y usar un puntero a nodo auxiliar ub_node que vaya guardando el nodo con la menor 
clave *mayor o igual a key*. Finalmente retorne el par del nodo ub\_node.
*/
...
Pair * upperBound(TreeMap * tree, void* key){
    ...
    if(ub_node != NULL){ // Si se encontró un nodo auxiliar
        tree->current = ub_node; // Actualiza el nodo actual
        return ub_node->pair; // Retorna el par encontrado
    }else{ // Si no se encontró un nodo auxiliar
        tree->current = NULL; // Actualiza el nodo actual a NULL
        return NULL; // Retorna NULL si no se encontró un upper bound
    }    
}
